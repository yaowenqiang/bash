...
DEFAULTDIR=/Opt     # 这里包含需要被刻录内容的目录。
                    # 必须保证目录存在。
                    # 小练习：测试一下目录是否存在。TODO
# 使用Joerg Schilling的"cdrecord"包
# http://www.fokus.fhg.de/usr/schilling/cdrecord.html

# 如果一般用户调用这个脚本的话，可能需要root身份，
# chmod u+x /usr/bin/cdrecord
...
# "l" 选项将给出一个"长"文件列表
# "R" 选项将使这个列表递归。
# "F" 选项将标记出文件类型(比如：目录以/结尾，而可执行文件以*结尾).
echo "Creating table of contents."

# 在烧录到CDR之前创建一个镜像文件。
mklsofs -r -o $IMAGEFILE $IMAGE_DIRECTORY
