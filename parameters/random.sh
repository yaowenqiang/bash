...
echo "Random number greater then $FLOOR --- $number"

echo

# 让我们对上边的循环尝试一个小发动，如下：
# let "number = $RANDOM  + $FLOOR "
# 这样不再需要那个while循环，并且能够运行的更快。
# 但是，这可能会产生一个问题，思考一下是什么问题?   TODO 还没思考

# 结合上边两个例子，来在指定的上下限之前来产生随机数。

number=0        # initialize
while [ "$number" -le "$FLOOR" ]
do
    number=$RANDOM
    let "number += $RANGE"      # 让number依比例落在$RANGE的范围内
done
...
number=$RANDOM
let "number %=$BINARY"
# 注意，let "number >>= 14" 将会给出一个更好的随机分配，#(译者注，正如man面中提到的，更高位的随机分布更加平均)
if [ "$number" -eq "$T" ]
then
    echo "TRUE"
else
...
die1=0
die2=0
# 是否让$POTS=7会比加1更好呢？解释行或者不行的原因？TODO 还末考虑
